<?php
	...
	//To insert into tbl_userreg
	if(isset($_POST['btnRegister']))
	{
		$insertQuery="insert into tbl_userreg (userFname,userLname,userDob,userEmail,userUsername,userPassword,userRegStatus) 
		value('".$firstname."','".$lastname."','".$dob."','".$email."','".$username."','".$password."',0)";
	...
	{
		$selQuery="select * from tbl_userreg";
		$res=mysql_query($selQuery);
		while($row=mysql_fetch_array($res))
			{
				if(($row['userUsername']==$uname)&&($row['userUsername']==$pword))
				{
					$_SESSION['LogeduserId']=$row['userId'];
					echo"<script>alert('login Successfull') </script>";
					header('location:../HomePage.php');
				}
				else if(($row['userUsername']==$uname)&&($row['userUsername']!=$pword))
				{
