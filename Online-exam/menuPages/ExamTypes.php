<?php
	...
	if($_GET['delId'])
	{
		$delQry="delete from tbl_testtype where testId=".$idNo."";
		//echo $delQry;
		mysql_query($delQry);
		echo"<script>alert('Successfully deleted')</script>";
		header('location:ExamTypes.php');
	}
?>
  ...
  <tbody>
   <?php
	  	while($row=mysql_fetch_array($ETypeRes))
		{
	  	?>
        <tr>
      <td value="<?php echo $row['testId'];?>"><?php echo $row['testName'];?></td>
      <td><a href="ExamTypes.php?delId=1&amp;id=<?php echo $row['testId'] ?>"><span class="delete">X</span></a></td>
    </tr>
      <?php
		}
		?>
