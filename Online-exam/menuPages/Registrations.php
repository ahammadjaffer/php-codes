<?php
	...
	$selQuery="select * from tbl_userreg u inner join tbl_registrations r on u.userId=r.userId inner join tbl_testtype y on r.testId=y.testId inner join tbl_adminshedule a on r.schId=a.schId where u.userRegStatus=1 and r.attendStatus=0";
	...
	if($_GET['delId'])
	{
		$updateQuery = "update tbl_registrations set attendStatus=1 where userId='".$idNo."'";
		//echo $delQry;
  ...
  <tbody>
   <?php
	  	while($row=mysql_fetch_array($res))
		{
	  	?>
        <tr>
      <td value="<?php echo $row['userId'];?>"><?php echo $row['userFname'];?></td>
      <td value="<?php echo $row['userId'];?>"><?php echo $row['testName'];?></td>
      <td value="<?php echo $row['userId'];?>"><?php echo $row['schDate'];?> &nbsp; <?php echo $row['schTime'];?></td>
      <td><a href="Registrations.php?delId=1&amp;id=<?php echo $row['userId'] ?>"><span class="delete">Approve</span></a></td>
    </tr>
      <?php
		}
		?>
